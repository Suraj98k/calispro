...
export function SkillCard({ skill, userMastery }: SkillCardProps) {
    ...
    const visual = getExerciseVisual(skill.category, skill.name);
    ...
    return (
        <Link
            href={`/dashboard/skills/${skillSlug}`}
        ...
        >
            <div className="flex items-start justify-between mb-4">
                <div className={cn(
                    ...
                    {visual.iconUrl ? (
                        <img src={visual.iconUrl} alt={`${skill.name} icon`} className="h-6 w-6 object-contain brightness-0 invert opacity-95" />
                    ) : (
