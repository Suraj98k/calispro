...
export default function TrackingPage() {
  ...
  return (
    <div className="min-h-screen space-y-6 px-4 py-6 md:px-8 md:py-8">
      ...
      <div className={cn('grid gap-5', showGuide ? 'xl:grid-cols-2' : 'grid-cols-1')}>
      {showGuide && (
        <aside className="app-surface h-fit overflow-hidden xl:sticky xl:top-20">
        ...
        <div className={cn('border-b border-border p-5', visual?.panelClassName || 'bg-surface-2')}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/10">
                {visual?.iconUrl ? (
                  <img src={visual.iconUrl} alt={`${exercise?.name || 'Exercise'} icon`} className="h-6 w-6 object-contain brightness-0 invert opacity-95" />
                ) : visual ? (
                  <visual.Icon className="h-6 w-6 text-primary" />
                ) : (
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-border bg-surface-2 px-3 py-1.5 text-[11px] font-semibold text-soft transition-colors hover:text-foreground"
            >
          ...
          {currentSets.map((set, index) => (
            <div
              ...
              className={cn(
                ...
                set.completed ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-border-subtle bg-surface-2/40'
              )}
            >
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-4">
                  <span className={cn('text-sm font-semibold', set.completed ? 'text-emerald-300' : 'text-white')}>Set {index + 1}</span>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={cn(
                      ...
                      set.completed ? 'bg-emerald-500 text-white' : 'border border-border bg-surface-3 text-soft',
              ...
              {timerActive && activeSetIndex === index && (
                <div className="mt-3 flex items-center justify-between rounded-lg border border-primary/30 bg-surface-1 px-3 py-2">
                  <div className="flex items-center gap-2">
                    <Timer className="h-4 w-4 text-primary" />
                    <p className="font-mono text-lg font-black tracking-tight text-white">{timerText}</p>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-md border border-border bg-surface-2 px-2.5 py-1 text-[11px] font-semibold text-soft hover:text-white"
                  >
